...
use App\Hesabe\Misc\PaymentHandler;
use App\Hesabe\Helpers\ModelBindingHelper;
use App\Hesabe\Libraries\HesabeCrypt;
...
class PaymentController
{
    ...
    public function formSubmit($request)
    {
        ...
        $paymentHandler = new PaymentHandler($this->paymentApiUrl, Constants::MERCHANT_CODE, $this->secretKey, $this->ivKey, $this->accessCode);
        // Getting the payment data into request object
        $requestData = $this->modelBindingHelper->getCheckoutRequestData($request);
...
// dd($hesabeCheckoutResponseModel->response['data']);
    ...
    {
        return "$this->paymentApiUrl/payment?data=$token";
    }
